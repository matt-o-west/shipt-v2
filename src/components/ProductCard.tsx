...
const ProductCard = ({ product }) => {
  ...
  const { id, fulfillmentSpeed, numberOfReviews, name, priceInfo, imageInfo } =
    product
    ...
    <div>
      <Link to={'/products/' + id}>
        <div className='relative max-w-80 h-44 p-4 mx-3 my-2 bg-opacity-80 backdrop-blur-sm animate-slideup rounded-sm cursor-pointer border hover:drop-shadow-xl hover:scale-105 hover:transition-scale hover:ease-in-out hover:duration-150 hover:backdrop-opacity-75'>
          <div className='absolute z-10 inset-0 bg-black/20 rounded-xl transition duration-300'>
            <img
              src={thumbnailUrl}
              alt={name}
      ...
      <button
        className='flex-nowrap bg-blue-500 border-blue-800 border hover:bg-blue-700 transition-all hover:ease-out text-white font-bold py-1 px-4 rounded-full white-space:nowrap mt-8'
